    ...
    @if ($expenses->isEmpty())
        @component('components.card')
            <div class="py-10 text-center">
                <p class="mb-6">Расходы не найдены <br> Нажмите на кнопку что бы добавить навые расходы</p>
                        ...
                        <td class="border-t">
                            <a href="{{ route('expenses.show', $expense->expense_date->format('Y-m-d')) }}"
                               class="px-2 text-blue-500 flex items-center justify-between cursor-pointer hover:text-blue-700">
                                Показать<span class="hidden md:block ml-1">Детали</span>
        ...
        <div class="w-1/2">
            @if ($expenses->previousPageUrl())
                <a href="{{ $expenses->previousPageUrl() }}"
                   class="inline-block font-bold shadow bg-white px-4 py-2 rounded-lg text-gray-600 hover:text-blue-600">Предыдущий</a>
            @endif
        </div>
        <div class="w-1/2 text-right">
            @if ($expenses->nextPageUrl())
                <a href="{{ $expenses->nextPageUrl() }}"
                   class="inline-block font-bold shadow bg-white px-4 py-2 rounded-lg text-gray-600 hover:text-blue-600">Следующий</a>
            @endif
        </div>
    </div>
</div>
